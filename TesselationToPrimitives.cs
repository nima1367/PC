...
namespace PrimitiveClassificationOfTessellatedSolids
{
    ...
    public static class TesselationToPrimitives
    {
        ...
        public static List<PrimitiveSurface> Run(TessellatedSolid ts)
        {
            ...
            if (unassignedEdges.Count == 0 || unassignedFaces.Count == 0) return new List<PrimitiveSurface>();
            // Classify Edges
            ...
            unassignedFaces = new HashSet<FaceWithScores>(unassignedFaces.Where(eachFace => eachFace.FaceCat.Any()));
            ...
            HashSet<EdgeWithScores> filteredOutEdges, HashSet<FaceWithScores> filteredOutFaces)
        {
            ...
            var badFaces2 = unassignedFaces.Where(f => f.Face.Edges.Any(e => e.OtherFace == null || e.OwnedFace == null)).ToList();
            foreach (var badFace in badFaces2)
            {
                filteredOutFaces.Add(badFace);
                unassignedFaces.Remove(badFace);
            }
            ...
            var badFaces3= unassignedFaces.Where(f => f.Face.Edges.Any(e => badEdges.Select(a=>a.Edge).Contains(e))).ToList();
            foreach (var badFace in badFaces3)
            {
                filteredOutFaces.Add(badFace);
                unassignedFaces.Remove(badFace);
            }
